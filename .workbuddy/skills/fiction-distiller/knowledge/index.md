...
- `short-substitution/*` 短篇换元仿写核心方法（v4.0新增）
- `anti-detection-short/*` 短篇反检测技术（v4.0新增）
- `word-count-control/*` 字数对齐与段落预算（v4.0新增）
- `originality-check/*` 原创性验证方法（v4.0新增）
...
| short-substitution/01-what-is-substitution | 短篇换元仿写定义与核心公式 | 借骨生肉、骨架层、变量层、人味层 | ~3500 |
| short-substitution/02-skeleton-extraction | 五维骨架提取方法 | 结构、节奏、视角、语言、情绪 | ~3500 |
| short-substitution/03-variable-list | 关节点与变量标注清单 | 人物、地点、事件、道具、关系、意象 | ~3500 |
| short-substitution/04-substitution-levels | 四层换元操作细则 | 词句、片段、框架、立意 | ~3500 |
| short-substitution/05-word-count-budget | 字数预算与段落控制 | 字数相似度、段落预算、伸缩技巧 | ~3500 |
| short-substitution/06-short-pipeline | 短篇七步工作流程 | 分析、提取、标注、设计、生成、对齐、验证 | ~3500 |
| anti-detection-short/01-detection-principles | AI检测原理与指标 | 困惑度、突发性、特征词、模式化标记 | ~3500 |
| anti-detection-short/02-humanization | 人味化注入技术 | 不完美、口语、重复、跳跃、留白 | ~3500 |
| anti-detection-short/03-rhythm-perturbation | 节奏扰动方法 | 长短句、段落不均、突然停顿 | ~3500 |
| anti-detection-short/04-ai-word-list | AI特征词与模式化标记黑名单 | 高频词、结构词、禁用表达 | ~3500 |
| anti-detection-short/05-short-verification | 短篇三关验证流程 | 原创性、字数、反检测 | ~3500 |
| word-count-control/01-count-analysis | 原文字数结构分析 | 总字数、段落、句子、对话占比 | ~3500 |
| word-count-control/02-budget-table | 字数预算表模板 | 段落预算、功能、误差 | ~3500 |
| word-count-control/03-expansion-compression | 字数扩写与压缩技巧 | 增删细节、拆分合并、对话调整 | ~3500 |
| originality-check/01-ngram-check | N-gram重合检测 | 3-gram、5-gram、重合率 | ~3500 |
| originality-check/02-variable-residue | 变量残留扫描 | 人物、地名、道具、连续相同字 | ~3500 |
| originality-check/03-human-review | 人工复核清单 | 检查点、通过标准、红线 | ~3500 |
...
short-substitution/01-what-is-substitution ──→ short-substitution/02-skeleton-extraction
    │                                              │
    ├──→ short-substitution/03-variable-list ──→ short-substitution/04-substitution-levels
    │
    ├──→ short-substitution/05-word-count-budget ──→ short-substitution/06-short-pipeline
    │
    ├──→ anti-detection-short/01-detection-principles ──→ anti-detection-short/05-short-verification
    │
    ├──→ word-count-control/01-count-analysis ──→ word-count-control/03-expansion-compression
    │
    └──→ originality-check/01-ngram-check ──→ originality-check/03-human-review
```

## 搜索覆盖率报告

- 已覆盖主题数：49 个（基础 32 + v4.0 新增 17）
